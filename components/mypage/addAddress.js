...
const addAddress = (props)  => {
    ...
    function onClickHandler(event) {
        ...
        if(event.target.name === 'submit'){
            ...
            axios.post(`http://localhost:4000/myPage/addAddress`,{params : {user_id: session.userId, name : name, phone : phone, address1 : address.address1, postCode : address.postCode, detail : detail, check : default_address}})
                .then((res) => {
        ...
        <div className='bg-[#22222280] fixed w-screen z-50 inset-0'>
            <div className='bg-white w-[520px] border border-black rounded-lg px-8 py-4 absolute top-1/2 left-1/2 translate-x-[-50%] translate-y-[-50%]' ref={closePop}>
                ...
                <div className={`my-4 ${name !== undefined && name.length<=1 ? 'text-[red]' : ''}`}>
                    ...
                    <input className={`text-[14px] font-[200] w-full  border-b-[1px] pb-1 ${name !== undefined  && name.length <= 1 ? 'outline-none border-b-[red]' : 'focus:login-focus'}`} placeholder='수령인의 이름' type='text' onChange={onChangeHandler} name='userName' autoComplete='off'/>
                    {name !== undefined && name.length <=1 ? <span className='text-[10px] text-[red]'>올바른 이름을 입력해주세요. (2 - 50자)</span> : ''}
                </div>
                <div className={`mb-4 ${phone !== undefined && phone.length<=9 ? 'text-[red]' : ''}`}>
                    <h2 className='text-[14px]'>
                        휴대폰 번호
                    </h2>
                    ...
                    {phone !== undefined && phone.length <=9 ? <span className='text-[10px] text-[red]'>정확한 휴대폰 번호를 입력해주세요.</span> : ''}
                </div>
                ...
                {popUp && (<Post setPopUp={setPopUp} setAddress={setAddress} address={address}/>)}
                <div className='border-b-[1px] mb-4'>
                    <h2 className='text-[14px]'>
                        주소
                    </h2>
                ...
                <div className='border-b-[1px] mb-4'>
                    ...
                    <input className='my-2 w-full text-[15px] font-[200] outline-none' placeholder='건물, 아파트, 동/호수 입력' onChange={onChangeHandler} name='detail' />
                </div>
                ...
                <div className='flex justify-center my-8'>
                    ...
                    <button className='border w-[120px] h-[42px] rounded-xl ml-2 text-[12px]' onClick={onClickHandler} name='submit'>저장하기</button>
                </div>
            </div>
        </div>
    )
